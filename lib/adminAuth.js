...
// يقارن كلمة المرور بأمان ثابت الزمن — بدون كشف الطول عبر early return.
// نستخدم HMAC بدل المقارنة المباشرة لضمان أن الإخراج دائماً نفس الطول.
export function checkPasscode(input) {
  const expected = process.env.ADMIN_PASSCODE;
  if (!expected) return false;
  const hmac = (v) => crypto.createHmac("sha256", "passcode-cmp").update(String(v)).digest();
  return crypto.timingSafeEqual(hmac(input), hmac(expected));
}
